...
for line in zhuce_file_1:
    ...
    # 修改获取的cookie，转化成jmeter能识别的格式
    cookies_new=str(cookies_new)
    cookies_new=cookies_new.replace(':','=')
    cookies_new=cookies_new.replace('{','')
    cookies_new = cookies_new.replace('}', '')
    cookies_new = cookies_new.replace("\'", '')
    cookies_new = cookies_new.replace(' ','')
    ...
    cookie_file=open('E:\软件测试第四阶段\\apache-jmeter-3.1\\bin\datafile\cookies.csv','a',encoding='utf-8')
    cookie_file.write(cookies_new+'\n')
